...
function GetVerification(token, done) {
  ...
  if (commonsignature === true || commonsignature === "true") {
    ...
    if (payload && payload.iss && payload.jti) {
      redisClient.get("token:iss:" + payload.iss + ":" + payload.jti, function (
        err,
        key
      ) {
        if (err) {
  ...
  User.findOne({ username: req.body.userName }, "+password", function (
    ...
    if (user && !user.Active) {
      return res.status(401).send({
        message:
          "User account deactivated, Please activate your account before login",
      });
    }
    ...
    Org.findOne({ companyName: req.body.companyName }, function (err, org) {
      ...
      if (org && org.companyEnabled) {
        ...
        return res
          .status(449)
          .send({ message: "Activate your organization before login" });
      }
    });
  });
};
    ...
    Org.findOne(
      ...
      function (err, org) {
        if (err) {
          return res
            .status(401)
            .send({ message: "Company verification failed" });
        }
  ...
  User.findOne({ username: req.body.userName }, "+password", function (
    ...
    if (config.auth.login_verification == true && !user.verified) {
      ...
      user.comparePassword(req.body.password, function (err, isMatch) {
        ...
        Org.findOne({ tenant: user.tenant, id: user.company }, function (
          err,
          org
        ) {
          if (err) {
...
module.exports.SignUP = function (req, res) {
  logger.info(
    "config.auth.signup_verification  -------->" +
      config.auth.signup_verification
  );
  if (
    config.auth.signup_verification == true ||
    config.auth.signup_verification == "true"
  ) {
    if (
      !req.body ||
    ...
    request(verificationUrl, function (error, response, body) {
      ...
      User.findOne({ username: req.body.mail }, function (err, existingUser) {
        ...
        user.save(function (err, result) {
          if (!err && result) {
            orgService.CreateOrganisationStanAlone(
              user,
      ...
      UserInvitation.findOne(
        ...
        function (err, invitation) {
          if (err || !invitation) {
            return res
              .status(404)
              .send({ message: "User Invitation Not Found" });
          } else {
                  ...
                  if (role && to && password) {
                    ...
                    if (limitObj) {
                      if (
                        ...
                        User.findOne({ username: to }, function (
                          err,
                          existingUser
                        ) {
                          if (err) {
                        ...
                        return res
                          .status(401)
                          .send({ message: "Limit has exceeded" });
                      }
                    } else {
                      return res
                        .status(401)
                        .send({ message: "Limit Not Found" });
                    }
                  } else {
                    return res
                      .status(404)
                      .send({ message: "Role or Password Not Found" });
                  }
                } else {
                  return res
                    .status(404)
                            ...
                            if (!err && result) {
                              ...
                              userAccount.save(function (err, account) {
                                if (err) {
                                  user.remove(function (err) {});
                                  return res.status(400).send({
                                    message: "Accountr Creation failed",
                                  });
                                } else {
                                  limitObj.currentAccess.push(user.username);
                                  Org.findOneAndUpdate(
      ...
      request.get(
        ...
        function (err, response, profile) {
          ...
          if (req.header("Authorization")) {
            User.findOne({ "googleplus.cid": profile.sub }, function (
              err,
              existingUser
            ) {
              if (existingUser) {
              ...
              GetVerification(token, function (err, payload) {
                if (!err && payload) {
                  ...
                  return res
                    .status(401)
                    .send({ message: "Token is not verified" });
                }
              });
            });
          } else {
              ...
              if (!user) {
                User.findOne({ "googleplus.cid": profile.sub }, function (
                  ...
                  if (existingUser) {
                    ...
                    user.company = 0;
                    user.tenant = 1;
                    (user.systemuser = true), (user.username = profile.email);
                        ...
                        orgService.CreateOrganisationStanAlone(
                          ...
                          function (err, rUser) {
                            if (!err && rUser) {
                              ...
                              res
                                .status(404)
                                .send({ message: "Organization save failed" });
                            }
                          }
                        );
                      } else {
...
module.exports.Google = function (req, res) {
  ...
  request.post(accessTokenUrl, { json: true, form: params }, function (
    ...
    if (token && companyName) {
          ...
          User.findOne({ username: profile.email }, function (err, user) {
            if (!user) {
              ...
              User.findOne({ "googleplus.cid": profile.sub }, function (
                ...
                if (existingUser) {
                  //return res.send({state: 'existing',token: GetJWT(existingUser, claims_arr)});
                  Org.findOne({ companyName: companyName }, function (
                    err,
                    org
                  ) {
                    if (err) {
                    ...
                    if (!org) {
                      return res
                        .status(401)
                        .send({ message: "Invalid organization name" });
                    }
                    ...
                    if (org && org.companyEnabled) {
                      UserAccount.findOne(
                        ...
                        function (err, account) {
                          ...
                          existingUser._doc.app_meta = account.app_meta;
                          existingUser._doc.user_scopes = account.user_scopes;
                          existingUser._doc.client_scopes =
                            account.client_scopes;
                          existingUser._doc.resourceid = account.resource_id;
                          existingUser._doc.veeryaccount = account.veeryaccount;
                          existingUser._doc.multi_login = account.multi_login;
      ...
      return res
        .status(401)
        .send({ message: "Token or CompanyName is not verified" });
    }
  });
};
...
module.exports.GitHub = function (req, res) {
  ...
  var companyName = req.body.companyname;
  // Step 1. Exchange authorization code for access token.
  request.get({ url: accessTokenUrl, qs: params }, function (
    err,
    response,
    accessToken
  ) {
    ...
    request.get(
      ...
      function (err, response, profile) {
        // Step 3a. Link user accounts.
        if (req.header("Authorization")) {
          User.findOne({ "github.cid": profile.sub }, function (
            err,
            existingUser
          ) {
            if (existingUser) {
          ...
          User.findOne({ username: profile.email }, function (err, user) {
            if (!user) {
              User.findOne({ "github.cid": profile.id }, function (
                err,
                existingUser
              ) {
                if (existingUser) {
...
module.exports.Facebook = function (req, res) {
  ...
  var graphApiUrl =
    "https://graph.facebook.com/v2.5/me?fields=" + fields.join(",");
  var params = {
    code: req.body.code,
    client_id: req.body.clientId,
  ...
  request.get({ url: accessTokenUrl, qs: params, json: true }, function (
    ...
    request.get({ url: graphApiUrl, qs: accessToken, json: true }, function (
      ...
      if (req.header("Authorization")) {
        User.findOne({ "facebook.cid": profile.id }, function (
          err,
          existingUser
        ) {
          if (existingUser) {
...
module.exports.ForgetPassword = function (req, res) {
  ...
  if (req.body.email) {
    User.findOne({ "email.contact": req.body.email }, function (
      err,
      existingUser
    ) {
      if (!existingUser || err) {
        ...
        crypto.randomBytes(20, function (err, buf) {
          ...
          redisClient.set("reset" + ":" + token, existingUser._id, function (
            err,
            val
          ) {
            if (err) {
...
module.exports.Attachments = function (req, res) {
  ...
  var item = {
    url:
      "http://fileservice.app.veery.cloud/DVP/API/1.0.0.0/InternalFileService/File/DownloadLatest/1/103/CDR_1_103_1476642600_1476728999.csv",
    name: "cdr.csv",
  };
