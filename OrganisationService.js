...
var AssignPackageToOrganisationLib = function (
  company,
  tenant,
  packageName,
  requestedUser,
    ...
    if (err) {
      ...
      if (vPackage) {
        Org.findOne({
          ...
          .populate("ownerRef", "-password")
          .exec(function (err, org) {
            if (err) {
              jsonString = messageFormatter.FormatMessage(
                err,
                "Find Organisation Failed",
                false,
                undefined
              );
              callback(jsonString);
              ...
              if (org) {
                if (org.ownerRef) {
                  UserAccount.findOne({
                    tenant: tenant,
                    company: company,
                    userref: org.ownerRef,
                  }).exec(function (err, userAccount) {
                    if (err) {
                      jsonString = messageFormatter.FormatMessage(
                        err,
                        "Find Organisation Owner Failed",
                        false,
                        undefined
                      );
                      callback(jsonString);
                      ...
                      if (org.tenantRef && org.tenantRef.rootDomain) {
                        ...
                        if (
                          ...
                          var typeExist = FilterObjFromArray(
                            org.packageDetails,
                          ...
                          if (
                            ...
                            if (
                                  ...
                                  if (err) {
                                    ...
                                    if (response) {
                                      if (response.IsSuccess) {
                                        try {
                                          org.packages.splice(
                                            org.packages.indexOf(
                                              typeExist.veeryPackage.packageName
                                            ),
                                            1
                                          );
                                        } catch (ex) {
                                          console.log(
                                            "No Package Found in the list:: ",
                                            ex
                                          );
                                        }
                                        org.packages.push(packageName);
                                        org.packages = UniqueArray(
                                          org.packages
                                        );
                                        typeExist.veeryPackage = vPackage._id;
                                        typeExist.buyDate = Date.now();

                                        SetPackageToOrganisation(
                                          company,
                                          tenant,
                                          domainData,
                                          vPackage,
                              ...
                              jsonString = messageFormatter.FormatMessage(
                                undefined,
                                "Cannot downgrade package, Please contact your system administrator",
                                false,
                                undefined
                              );
                              callback(jsonString);
...
var SetPackageToOrganisation = function (
  company,
  tenant,
  domainData,
  vPackage,
  ...
  er.on("endExtractResources", function (userScopes) {
    ...
    Org.findOneAndUpdate({ tenant: tenant, id: company }, org, function (
      err,
      rOrg
    ) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          err,
          "Assign Package to Organisation Failed",
          false,
          undefined
        );
      } else {
...
function AssignTaskToOrganisation(company, tenant, taskList) {
  var taskInfoUrl = util.format(
    "http://%s/DVP/API/%s/ResourceManager/TaskInfo",
    config.Services.resourceServiceHost,
    config.Services.resourceServiceVersion
  );
  var taskUrl = util.format(
    "http://%s/DVP/API/%s/ResourceManager/Task",
    config.Services.resourceServiceHost,
    config.Services.resourceServiceVersion
  );
  if (
    ...
    taskUrl = util.format(
      "http://%s:%s/DVP/API/%s/ResourceManager/Task",
      config.Services.resourceServiceHost,
      config.Services.resourceServicePort,
      config.Services.resourceServiceVersion
    );
    taskInfoUrl = util.format(
      "http://%s:%s/DVP/API/%s/ResourceManager/TaskInfo",
      config.Services.resourceServiceHost,
      config.Services.resourceServicePort,
      config.Services.resourceServiceVersion
    );
  ...
  restClientHandler.DoGet(companyInfo, taskInfoUrl, "", function (
    err,
    res1,
    result
  ) {
    if (err) {
      ...
      for (var i in taskList) {
        ...
        if (task) {
          ...
          restClientHandler.DoPost(companyInfo, taskUrl, body, function (
            err,
            res1,
            result
          ) {
...
function AssignContextAndCloudEndUserToOrganisation(company, tenant, domain) {
  var contextUrl = util.format(
    "http://%s/DVP/API/%s/SipUser/Context",
    config.Services.sipuserendpointserviceHost,
    config.Services.sipuserendpointserviceVersion
  );
  var transferCodesUrl = util.format(
    "http://%s/DVP/API/%s/SipUser/TransferCode",
    config.Services.sipuserendpointserviceHost,
    config.Services.sipuserendpointserviceVersion
  );
  var cloudEndUserUrl = util.format(
    "http://%s/DVP/API/%s/CloudConfiguration/CloudEndUser",
  ...
  if (
    ...
    transferCodesUrl = util.format(
      "http://%s:%s/DVP/API/%s/SipUser/TransferCode",
      config.Services.sipuserendpointserviceHost,
      config.Services.sipuserendpointservicePort,
      config.Services.sipuserendpointserviceVersion
    );
  ...
  restClientHandler.DoPost(companyInfo, contextUrl, contextReqBody, function (
    err,
    res1,
    result
  ) {
    if (err) {
      ...
      restClientHandler.DoPost(
        companyInfoForCloudEndUser,
        cloudEndUserUrl,
        cloudEndUserReqBody,
        ...
        Console.findOne({ consoleName: consoleName }, function (err, rConsole) {
          if (err) {
            ...
            if (rConsole) {
              ...
              var consoleScope = {
                consoleName: rConsole.consoleName,
                menus: [],
              };
              if (rConsole.consoleNavigation) {
                for (var j = 0; j < rConsole.consoleNavigation.length; j++) {
                  ...
                  if (
                    ...
                    navigation.navigationTypes.indexOf(navigationType) > -1
                  ) {
                    var menuScope = {
                      menuItem: navigation.navigationName,
                      menuAction: [],
                    };
                    for (var k = 0; k < navigation.resources.length; k++) {
                      ...
                      if (navigationResource && navigationResource.scopes) {
                        for (
                          var l = 0;
                          l < navigationResource.scopes.length;
                          l++
                        ) {
                          var navigationResourceScope =
                            navigationResource.scopes[l];
                          if (navigationResourceScope) {
                            ...
                            if (navigationResourceScope.actions) {
                              for (
                                var m = 0;
                                m < navigationResourceScope.actions.length;
                                m++
                              ) {
        ...
        if (uScopes) {
          for (var i = 0; i < uScopes.length; i++) {
            ...
            if (eUserScope) {
              if (
                uScopes[i].read &&
                (!eUserScope.read || eUserScope.read == false)
              ) {
                eUserScope.read = uScopes[i].read;
              }
              if (
                uScopes[i].write &&
                (!eUserScope.write || eUserScope.write == false)
              ) {
                eUserScope.write = uScopes[i].write;
              }
              if (
                uScopes[i].delete &&
                (!eUserScope.delete || eUserScope.delete == false)
              ) {
        ...
        ec.on("endExtractConsoles", function (clientScopes) {
          if (clientScopes) {
            for (var j = 0; j < clientScopes.length; j++) {
              if (
                userAccount.client_scopes &&
                userAccount.client_scopes.length > 0
              ) {
                var existingClientScope = FilterObjFromArray(
                  userAccount.client_scopes,
                  "consoleName",
                  clientScopes[j].consoleName
                );
          ...
          userAccount.client_scopes = UniqueObjectArray(
            userAccount.client_scopes,
            "consoleName"
          );
          ...
          userAccount.user_scopes = UniqueObjectArray(
            userAccount.user_scopes,
            "scope"
          );
...
function CreateOrganisationStanAlone(user, companyname, timezone, callback) {
  ...
  GetNewCompanyId(function (cid) {
    if (cid && cid > 0) {
      if (user.company === 0) {
        logger.info(
          "DVP-UserService.CreateOrganisationStanAlone Active Tenant: " +
            config.Tenant.activeTenant
        );
        Tenant.findOne({ id: config.Tenant.activeTenant }, function (
          err,
          Tenants
        ) {
          if (err) {
...
function RequestToBill(company, tenant, billInfo, callback) {
  try {
    var contextUrl = util.format(
      "http://%s/DVP/API/%s/Billing/BuyPackage",
      config.Services.billingserviceHost,
      config.Services.billingserviceVersion
    );
    if (
      config.Services.dynamicPort ||
      validator.isIP(config.Services.billingserviceHost)
    ) {
      contextUrl = util.format(
        "http://%s:%s/DVP/API/%s/Billing/BuyPackage",
        config.Services.billingserviceHost,
        config.Services.billingservicePort,
        config.Services.billingserviceVersion
      );
    }
    var companyInfo = util.format("%d:%d", tenant, company);
    restClientHandler.DoPost(companyInfo, contextUrl, billInfo, function (
      err,
      res1,
      result
    ) {
